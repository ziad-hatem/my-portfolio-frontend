...
export default function ProfilesAdminPage() {
  ...
  const searchProfile = async () => {
    ...
    try {
      const res = await fetch(`/api/profile?userId=${encodeURIComponent(searchUserId)}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400">Track and analyze user behavior across your site</p>
        </div>
              ...
              <div className="space-y-2">
                {analytics.topCountries.slice(0, 5).map((item, index) => (
                  <div key={index} className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-blue-400">{item.count}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <div className="space-y-2">
                {analytics.topPages.slice(0, 5).map((item, index) => (
                  <div key={index} className="flex justify-between items-center">
                    <span className="text-gray-300 truncate max-w-[200px]" title={item.page}>
                      {item.page}
                    </span>
              ...
              <div className="space-y-2">
                {analytics.topDevices.map((item, index) => (
                  <div key={index} className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-green-400">{item.count}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {selectedProfile && (
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 mb-8">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-2xl font-bold mb-2">User Profile Details</h3>
                <p className="text-gray-400 font-mono text-sm">{selectedProfile.userId}</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <InfoCard label="Total Visits" value={selectedProfile.totalVisits.toString()} />
              <InfoCard
                label="Page Views"
            ...
            {selectedProfile.pageViews.length > 0 && (
              <div className="mb-6">
                <h4 className="text-lg font-semibold mb-3">Recent Page Views</h4>
                <div className="space-y-2">
                  {selectedProfile.pageViews.slice(0, 10).map((pv, index) => (
                    <div key={index} className="bg-gray-900 rounded-lg p-3 flex justify-between">
                      <div>
            ...
            {selectedProfile.interactions.length > 0 && (
              <div>
                <h4 className="text-lg font-semibold mb-3">Recent Interactions</h4>
                <div className="space-y-2">
                  {selectedProfile.interactions.slice(0, 10).map((interaction, index) => (
                    <div key={index} className="bg-gray-900 rounded-lg p-3 flex justify-between">
                      <div>
                        <p className="font-semibold capitalize">
                          {interaction.type.replace('_', ' ')}
                        </p>
                        <p className="text-sm text-gray-400">{interaction.element || interaction.page}</p>
                      </div>
                      <div className="text-right text-sm text-gray-400">
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-1">
                    <p className="font-mono text-sm text-blue-400 mb-1">{profile.userId}</p>
...
function StatCard({
  ...
  return (
    <div className={`bg-gradient-to-br ${colorMap[color as keyof typeof colorMap]} rounded-xl p-6 border`}>
